...
<script lang="ts">
	...
	async function onDropFiles(files: FileList | undefined): Promise<DropFileResult[]> {
		...
		if (failed.length > 0) {
			...
			showError('文件上传失败', failed.map((result) => result.reason).join(', '));
		}
	...
	const generateMessages = $derived.by(() => ({
		commit: '生成提交信息',
		pr: `生成 ${reviewUnitAbbr ?? 'PR'} 描述`
	}));

	function getTooltipText(): string | undefined {
		if (!canUseAI) {
			return '需要在项目设置中启用 AI 才能使用此功能';
		}
...
{#snippet buttonText()}
	{currentEditorWidth > DROPDOWN_BTN_BREAKPOINTS.medium ? '生成信息' : '生成'}
{/snippet}
...
>
	你的文件将存储在 GitButler 的数字保险库中，安全可靠。我们保证它是安全的，所以你可以随意分享链接 🔐
	{#snippet controls(close)}
		<div class="modal-footer">
			...
			<Button style="pop" type="submit">是的，上传！</Button>
		</div>
	{/snippet}
</Modal>
		...
		<div class="message-textarea__toolbar">
			<div class="message-textarea__toolbar__left">
				<Button
					...
					tooltip={useFloatingBox.current ? '退出悬浮模式' : '使用悬浮模式'}
					onclick={() => {
