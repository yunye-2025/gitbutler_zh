<script lang="ts">
	...
	const forgeName = $derived(FORGE_NAME_MAP[forge.determinedForgeType] ?? '你的代码托管平台');
				...
				<ScrollableContainer>
					...
					{:else}
						...
						{#if groupedBranches.review.length > 0}
							{@render branchGroup({ title: '请求审查', children: groupedBranches.review })}
						{/if}
			...
			<EmptyStatePlaceholder image={noBranchesSvg} width={EMPTY_STATE_WIDTH} bottomMargin={48}>
				...
				{#snippet caption()}
					{#if selectedOption === 'pullRequest'}
						{#if baseBranch}
							未找到来自 <strong>{baseBranch.remoteName}</strong> 的 {forge.reviewUnitAbbr}。
						{:else}
							未找到 {forge.reviewUnitAbbr}。
						{/if}
					{:else if selectedOption === 'local'}
						创建新分支或从远程获取。
					{:else if baseBranch}
						来自 <strong>{baseBranch.remoteName}/{baseBranch.shortName}</strong> 的分支和
						{forge.reviewUnitAbbr} 将显示在此处。
					{/if}
					{#if shouldShowAuthMessage}
						使用 {forgeName} 完成认证以查看 {forge.reviewUnitAbbr}。
					{/if}
				{/snippet}
			</EmptyStatePlaceholder>
		</div>
