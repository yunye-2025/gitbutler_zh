<script lang="ts">
	...
	const checksTagInfo: StatusInfo = $derived.by(() => {
		...
		if (checks) {
			...
			const reducedText = checks.completed ? (checks.success ? '通过' : '失败') : '进行中';
			return { style, icon, text, reducedText, tooltip };
		}
		if (loading) {
			return { style: 'neutral', icon: 'spinner', text: '检查', reducedText: '检查' };
		}

		return { style: 'neutral', icon: undefined, text: '无 PR 检查', reducedText: '无检查' };
	});
