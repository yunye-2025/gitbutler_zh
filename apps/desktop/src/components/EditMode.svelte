		...
		{#snippet children(project)}
			<div class="editmode__container">
				<h2 class="editmode__title text-18 text-body text-bold">
					你正在编辑提交 <span class="code-string">
						{editModeMetadata.commitOid.slice(0, 7)}
					</span>
					<InfoButton title="编辑模式">
						编辑模式允许你独立修改现有提交或解决冲突。所有更改（包括新文件）都会添加到所选提交中。
					</InfoButton>
				</h2>
				...
				<div class="commit-group">
					...
					<div bind:this={filesList} class="card files">
						<div class="header" class:show-border={isCommitListScrolled}>
							<h3 class="text-15 text-semibold">提交文件</h3>
				...
				<div class="editmode__actions">
					...
					<Button kind="outline" onclick={abort} disabled={loading} {loading}>取消</Button>
					<Button
...
>
	<p class="text-13 text-body helper-text">仍有一些文件可能存在冲突。确定要保存并退出吗？</p>
	{#snippet controls(close)}
		<Button kind="outline" type="reset" onclick={close}>取消</Button>
		<Button style="danger" type="submit" {loading}>保存并退出</Button>
	{/snippet}
</Modal>
