...
{#if shouldShow}
	...
	>
		<div class="dialog-popup__header">
			...
			<h3 class="text-12 text-bold flex-1">搜索文件</h3>
		...
		{#if loading}
			<div class="text-12 text-italic dialog-popup__placeholder-content">加载中...</div>
		{:else if files && files.length === 0 && query.length === 0}
			<div class="text-12 text-italic dialog-popup__placeholder-content">
				输入内容开始搜索…
			</div>
		{:else if files && files.length === 0}
			<div class="text-12 dialog-popup__placeholder-content">未找到文件 ¯\_(ツ)_/¯</div>
		{:else if files}
