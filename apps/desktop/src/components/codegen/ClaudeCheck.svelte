	...
	<div class="flex items-center gap-8 m-b-6">
		<div class="flex items-center gap-8 flex-1">
			{#if isClaudeNotAvailable}
				...
				<h4 class="text-16 text-semibold text-body">找不到 Claude Code</h4>
			{:else}
				<Icon name="success" color="safe" />
				<h4 class="text-16 text-semibold text-body">Claude Code 已连接</h4>
			{/if}
		</div>
	</div>
{/if}
...
<div class="claude-config">
	<Textbox
		...
		placeholder="Claude Code 可执行文件路径"
	...
	{#if availabilityQuery.response?.status === 'execution_failed' && (availabilityQuery.response.stdout || availabilityQuery.response.stderr)}
		<div class="stack-v gap-8">
			{#if availabilityQuery.response.stdout}
				<Codeblock label="标准输出" content={availabilityQuery.response.stdout} />
			{/if}
			{#if availabilityQuery.response.stderr}
				<Codeblock label="标准错误" content={availabilityQuery.response.stderr} />
			{/if}
		</div>
	{/if}
