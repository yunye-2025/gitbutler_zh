<script lang="ts">
	...
	const thinkingLevels: { label: string; shortLabel: string; value: ThinkingLevel }[] = [
		{ label: '普通', shortLabel: '普通', value: 'normal' },
		{ label: '深度思考', shortLabel: '思考', value: 'think' },
		{ label: '超深思考', shortLabel: '超深', value: 'megaThink' },
		{ label: '极限思考', shortLabel: '极限', value: 'ultraThink' }
	];

	const permissionModeOptions: { label: string; value: PermissionMode }[] = [
		{ label: '需授权编辑', value: 'default' },
		{ label: '规划', value: 'plan' },
	...
	<ReduxResult result={claudeAvailable.result}>
		{#snippet loading()}
			<PreviewHeader {onclose}>
				{#snippet content()}
					<h3 class="text-14 text-semibold truncate">分支 {branchName} 的聊天</h3>
				{/snippet}
				{#snippet actions()}
			...
			<PreviewHeader {onclose}>
				...
				{#snippet actions()}
					...
					<div class="flex gap-10 items-center">
						{#if stats.tokens > 0}
							<Tooltip text="令牌：{stats.tokens.toLocaleString()} / ${stats.cost.toFixed(2)}">
								<span class="text-12 clr-text-2">
									{formatCompactNumber(stats.tokens)}
								</span>
							</Tooltip>

							<Tooltip text="已使用 {contextUsage}% 上下文">
								<div
					...
					<ConfigurableScrollableContainer childrenWrapDisplay="contents">
						<div class="no-agent-placeholder">
							<div class="no-agent-placeholder__content">
								...
								<h2 class="text-serif-42">连接 Claude Code</h2>
								<p class="text-13 text-body clr-text-2">
									如果你尚未安装 Claude Code，请查看我们的 <Link
										class="clr-text-1"
										href="https://docs.gitbutler.com/features/agents-tab#installing-claude-code"
										>安装指南</Link
									>.
									<br />
									点击下方按钮检查 Claude Code 是否可用。
								</p>
							...
							<p class="text-12 text-body clr-text-2">
								连接遇到问题？
								<br />
								查看 <Link href="https://docs.claude.com/en/docs/claude-code/troubleshooting"
									>故障排查指南</Link
								> 了解常见问题与解决方案。
							</p>
						</div>
...
>
	确定要清空上下文并删除与此 Claude 会话关联的所有规则吗？此操作无法撤销。

	{#snippet controls(close)}
		<Button kind="outline" onclick={close}>取消</Button>
		<Button style="danger" type="submit">清空上下文</Button>
	{/snippet}
</Modal>
