...
<div class="codegen-entry-wrapper" use:focusable={{ focusable: true, onAction: () => onclick?.() }}>
		...
		{#if commitCount > 0}
			<div class="commits-drawer">
				<button class="commits-drawer-header" onclick={() => (isOpen = !isOpen)} type="button">
					...
					<p class="text-13 text-semibold">提交</p>
	...
	{#if totalHeads > 1}
		<div class="entry-heads">
			...
			<p class="text-12 text-semibold full-width">Stack 中还有 {totalHeads - 1} 个分支</p>

			<InfoButton>
				目前 GitButler 不支持为同一个 Stack 开启多个会话。会话只会应用到顶部分支。
			</InfoButton>
		</div>
	{/if}
</div>
