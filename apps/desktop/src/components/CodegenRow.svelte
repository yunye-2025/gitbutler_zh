...
<Dropzone {handlers}>
	{#snippet overlay({ hovered, activated })}
		<CardOverlay {hovered} {activated} label="引用" />
	{/snippet}
	...
	{#if draft}
		...
		<div class="codegen-row">
			...
			<h3 class="text-13 text-semibold truncate codegen-row__title">AI 会话将在此开始</h3>
		</div>
		...
		<ReduxResult {projectId} result={combineResults(messages.result, permissionRequests.result)}>
			{#snippet children([messages, permissionReqs])}
				...
				>
					...
					{#if hasPendingApproval}
						<Badge style="pop" tooltip="等待批准">需要操作</Badge>
					{/if}

					{#if totalCount > 1}
						<span class="text-12 codegen-row__todos">待办（{completedCount}/{totalCount}）</span>
