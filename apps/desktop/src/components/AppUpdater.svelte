...
{#if version || status === 'Up-to-date'}
	<div class="update-banner" data-testid="update-banner" class:busy={$loading}>
		...
		<div class="buttons">
			{#if releaseNotes}
				<Button kind="outline" onclick={handleOpenModal}>更新说明</Button>
			{/if}
			{#if !inFlatpak}
			...
			<div class="p-16">
				{#if loadingReleases}
					<div class="loading-state">
						<p class="text-12">正在加载版本...</p>
					</div>
				{:else}
					<div class="release-notes-header">
						<h3 class="text-15 text-bold">
							<span class="text-12 m-r-4">📒</span> 更新说明 - {displayVersion}
						</h3>
					...
					<div class="text-12 text-body release-notes-content">
						<Markdown content={displayNotes || '暂无更新说明'} />
					</div>
				{/if}
			</div>
		</ConfigurableScrollableContainer>
