		...
		<form onsubmit={onSubmit} class="profile-form">
			...
			<div id="contact-info" class="contact-info">
				<div class="contact-info__fields">
					<Textbox label="姓名" bind:value={newName} required />
					<Textbox label="邮箱" bind:value={$user.email} readonly />
				</div>

				<Button type="submit" style="pop" loading={saving}>更新资料</Button>
			</div>
		</form>
	</CardGroup>
	...
	<CardGroup.Item>
		...
		{#snippet caption()}
			{#if platformName === 'windows'}
				在 Windows 上，你可以手动将可执行文件（<code>`but`</code>）复制到 PATH 目录中。
				点击“显示命令”获取指引。
			{:else}
				将 GitButler CLI（<code>`but`</code>）安装到 PATH 中，便于在终端使用。
				此操作需要管理员权限。你也可以手动创建符号链接。
			{/if}
		{/snippet}
...
>
	...
	{#snippet controls(close)}
		<Button style="error" kind="outline" loading={isDeleting} type="submit">移除</Button>
