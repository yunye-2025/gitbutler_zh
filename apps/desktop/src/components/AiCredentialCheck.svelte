		...
		try {
			...
			if (!isConfigValid) {
				if (modelKind === ModelKind.OpenAI || modelKind === ModelKind.Anthropic) {
					if (isUsingButlerAPI && !$user) {
						...
						throw new Error('请为所选的 AI 服务提供有效的 API 密钥');
					}
					...
					throw new Error(
						`请检查 Ollama 配置：endpoint=${endpoint}, model=${model}`
					);
			...
			testTimeout = setTimeout(() => {
				if (testing) {
					...
					error =
						'AI 响应超时（20 秒）。请检查你的 AI 服务是否正常运行。';
					testing = false;
			...
			debugInfo += `, 收到提交信息: ${result?.substring(0, 30)}${result && result.length > 30 ? '...' : ''}`;
					...
					<div class="result-content" transition:slide={{ duration: 250 }}>
						{#if error}
							...
							{:else if modelKind === ModelKind.OpenAI || modelKind === ModelKind.Anthropic}
								<span> 请检查你的 API 密钥或尝试使用 GitButler 的 API。 </span>
							{:else if modelKind === ModelKind.Ollama}
								<span>
									请检查 Ollama 的端点与模型配置。
									<br />
									请确保 Ollama 在本地运行且可访问。
