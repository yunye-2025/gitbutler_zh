		...
		{#if isChangedFilesItem(item)}
			{#if isChangedFolderItem(item)}
				<p class="discard-caption">
					确定要丢弃 <span class="text-bold">{item.path}</span> 中的所有更改吗？
				</p>
			{:else}
				{@const changes = item.changes}
				...
				{:else}
					<p>
						要丢弃全部 <span class="text-bold">{changes.length} 个文件</span> 的更改吗？
					</p>
				{/if}
			{/if}
		{:else}
			...
			<div class="explanation">
				<p class="primary-text">
					...
					{:else}
						你选择的更改
					{/if}
					将被移动到新分支并从当前工作区移除。需要找回这些更改时，切换到新分支并撤销暂存。
				</p>
			</div>

			<div class="technical-note">
				<p class="text-12 text-body clr-text-2">
					💡 这会创建一个新分支、提交你的更改，然后取消应用该分支。后续版本将提供更简单的暂存管理。
				</p>
			</div>
		</div>
	{/snippet}
