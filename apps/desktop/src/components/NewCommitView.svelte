<script lang="ts">
	...
	async function createCommit(message: string) {
		if (isCooking) {
			showToast({ message: '提交正在进行中', style: 'danger' });
			return;
		}
		...
		try {
			...
			if (!finalStackId) {
				throw new Error('未选择堆叠！');
			}

			if (!finalBranchName) {
				throw new Error('未选择分支！');
			}
			...
			if ($runCommitHooks) {
				...
				if (messageHookResult?.status === 'failure') {
					showError('提交信息钩子失败', messageHookResult.error);
					return;
	...
	async function handleCommitCreation(title: string, description: string) {
		...
		if (!message) {
			showToast({ message: '提交信息不能为空', style: 'danger' });
			return;
		}
