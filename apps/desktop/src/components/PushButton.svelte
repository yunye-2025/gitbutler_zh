<script lang="ts">
	...
	function getButtonTooltip(
		...
		if (hasConflicts) {
			return '请先解决有冲突的提交后再推送。';
		}

		if (multipleBranches && !isLastBranchInStack) {
			return '推送此分支及其下方所有分支';
		}

		if (withForce) {
			return remoteTrackingBranch
				? '强制推送此分支'
				: `强制推送此分支到 ${remoteTrackingBranch}`;
		}

		return remoteTrackingBranch
			? `推送此分支到 ${remoteTrackingBranch}`
			: '推送此分支';
	}
	...
	{#snippet children([branchDetails, branches])}
		...
		>
			<p class="description">
				...
				>
				而你的本地分支未包含这些提交。为避免覆盖历史，请<span class="text-bold">取消并拉取与集成</span
				>这些更改。
			</p>
			{#if upstreamCommits}
