<script lang="ts">
	...
	const title = $derived.by(() => {
		...
		return `将 ${targetProject.value.name} 加入 ${chosenOrganization.value.name}`;
	});
...
<Modal bind:this={modal} {title}>
			...
			<CardGroup.Item>
				<Loading loadable={organizationProject}>
					{#snippet children(organizationProject)}
						...
						<Button onclick={() => connectToOrganization(organizationProject.slug)}>连接</Button>
					{/snippet}
				</Loading>
			</CardGroup.Item>
		{/each}
	...
	<Button onclick={() => connectToOrganization()}>创建组织项目</Button>
</Modal>

<Button onclick={() => modal?.show()}>连接</Button>
