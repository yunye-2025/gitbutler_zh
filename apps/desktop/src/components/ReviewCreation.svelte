...
<script lang="ts">
	...
	async function createPr(params: CreatePrParams): Promise<PullRequest | undefined> {
		...
		try {
			if (!baseBranchName) {
				chipToasts.error('未确定基分支名称');
				return;
			}

			if (!params.upstreamBranchName) {
				chipToasts.error('未确定上游分支名称');
				return;
			}
			...
			chipToasts.success(`${unit} #${pr.number} 创建成功`);
...
<div class="pr-editor">
	...
	<div class="pr-fields">
		<MessageEditorInput
			...
			placeholder="{forge.reviewUnitAbbr} 标题"
			showCount={false}
		...
		<MessageEditor
			...
			enableFileUpload
			enableSmiles
			placeholder="{forge.reviewUnitAbbr} 描述"
			messageType="pr"
