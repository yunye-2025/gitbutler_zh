<script lang="ts">
	...
	async function gitHubOauthCheckStatus(deviceCode: string) {
		...
		try {
			...
			toasts.success('已验证 GitHub 账户');
		} catch (err: any) {
			console.error(err);
			toasts.error('验证 GitHub 账户失败');
	...
	async function storePersonalAccessToken() {
		...
		try {
			...
			console.error('保存 GitHub PAT 失败：', err);
			patError = '令牌无效或网络错误';
	...
	async function storeGitHubEnterpriseToken() {
		...
		try {
			...
			console.error('保存 GitHub Enterprise PAT 失败：', err);
			ghePatError = '令牌无效或主机无效';
		...
		<CardGroup>
			...
			<CardGroup.Item>
				<div class="flex justify-end gap-6">
					<Button style="gray" kind="outline" onclick={cleanupPatFlow}>取消</Button>
					<Button
						style="pop"
						disabled={!patInput}
	...
	{:else if showingFlow === 'ghe'}
		<CardGroup>
			<CardGroup.Item>
				...
				{#snippet caption()}
					要连接你的 GitHub Enterprise API，请在应用的 CSP 设置中将其加入允许列表。
					<br />
				...
				<Textbox
					...
					helperText="这应为 API 的根 URL。例如，如果你的 GitHub Enterprise Server 主机名是 github.acme-inc.com，则将基础 URL 设置为 https://github.acme-inc.com/api/v3"
					error={gheHostError}
				/>
				<Textbox
					label="个人访问令牌"
			...
			<CardGroup.Item>
				<div class="flex justify-end gap-6">
					<Button style="gray" kind="outline" onclick={cleanupGheFlow}>取消</Button>
					<Button
						style="pop"
