	...
	function mapOperation(snapshotDetails: SnapshotDetails | undefined): {
		...
		switch (snapshotDetails.operation) {
			...
			case 'UpdateBranchRemoteName':
				return {
					text: `更新分支远程名称 "${snapshotDetails.trailers.find((t) => t.key === 'before')?.value}" 为 "${snapshotDetails.trailers.find((t) => t.key === 'after')?.value}"`,
					icon: 'item-slash'
				};
			case 'SetBaseBranch':
			...
			case 'UpdateWorkspaceBase':
				return { text: '更新工作区基准', icon: 'rebase' };
			case 'EnterEditMode':
	...
	<div class="snapshot-right-container">
		<div class="restore-btn">
			<Button
				size="tag"
				kind="outline"
				tooltip="将 GitButler 和你的文件恢复到此操作之前的状态。回退操作也可撤销。"
				onclick={() => {
					onRestoreClick();
				}}
				disabled={mode.response?.type !== 'OpenWorkspace'}>回退</Button
			>
		</div>
