...
<SettingsSection>
	<div class="clone__field repositoryUrl">
		<div class="text-13 text-semibold clone__field--label">克隆 URL</div>
		<Textbox bind:value={repositoryUrl} />
	</div>
	<div class="clone__field repositoryTargetPath">
		<div class="text-13 text-semibold clone__field--label">克隆到</div>
...
{/if}
{#if errors.length}
	{@render Notification({ title: '错误', items: errors, style: 'error' })}
{/if}
