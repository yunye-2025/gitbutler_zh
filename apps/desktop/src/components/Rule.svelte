	...
	{#if target.type === 'stackId'}
		...
		<ReduxResult {projectId} result={stack.result}>
			{#snippet children(stack)}
				...
				{:else}
					{@render stackPill('error-small', '分支缺失', '未找到关联的堆叠', true)}
				{/if}
			{/snippet}
		</ReduxResult>
	{:else if target.type === 'leftmost'}
		{@render stackPill('leftmost-lane', '最左侧', '最左侧泳道')}
	{:else if target.type === 'rightmost'}
		{@render stackPill('rightmost-lane', '最右侧', '最右侧堆叠')}
	{/if}
{/snippet}
...
>
	...
	{#snippet controls(close)}
		...
		<Button loading={deletingRule.current.isLoading} style="error" type="submit">删除规则</Button
		>
	{/snippet}
</Modal>
