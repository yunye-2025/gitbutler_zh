...
export function getBranchStatusLabelAndColor(pushStatus: PushStatus): {
	...
	switch (pushStatus) {
		case 'completelyUnpushed':
			return { label: '未推送', color: colorMap.LocalOnly };
		case 'nothingToPush':
			return { label: '无可推送', color: colorMap.LocalAndRemote };
		case 'unpushedCommits':
		case 'unpushedCommitsRequiringForce':
			return { label: '部分未推送', color: colorMap.LocalAndRemote };
		case 'integrated':
			return { label: '已集成', color: colorMap.Integrated };
		default:
			return { label: '未知', color: colorMap.Error };
	}
}
