...
export function handleCreateBranchFromBranchOutcome(outcome: CreateBranchFromBranchOutcome) {
	if (outcome.unappliedStacks.length > 0) {
		showToast({
			...
			title: `提示：为应用此分支，我们已取消应用${stackCount(outcome.unappliedStacks.length)}`,
			message: `在将此分支应用到你的工作区时检测到冲突，因此我们已自动取消应用${prettyNamedListIfPossible(outcome.unappliedStacks.length, outcome.unappliedStacksShortNames)}。
你可以稍后在分支页面重新应用它们。`
		});
	}
}
...
export function handleMoveBranchResult(result: MoveBranchResult) {
	if (result.unappliedStacks.length > 0) {
		showToast({
			testId: TestId.StacksUnappliedToast,
			title: '提示：为移动此分支，我们已取消应用部分堆叠',
			message: `看起来该分支无法与其他 ${result.unappliedStacks.length} 个堆叠一起干净地应用。
你可以稍后在分支页面重新应用它们。`
		});
	}
}
