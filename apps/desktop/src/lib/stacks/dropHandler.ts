	...
	async ondropChangeData(data: ChangeDropData) {
		switch (data.selectionId.type) {
			case 'branch': {
				...
				if (!newBranchName) {
					throw new Error('无法生成新的分支名称。');
				}

				if (!data.stackId) {
					throw new Error('变更拖放数据必须指定源堆叠 ID');
				}
		...
		switch (data.selectionId.type) {
			case 'commit': {
				if (!data.stackId || !data.commitId) {
					throw new Error('代码块拖放数据必须指定源堆叠 ID 和提交 ID');
				}
			...
			case 'worktree': {
				...
				if (!assignment) {
					throw new Error('未找到已拖放工作区代码块的分配信息');
				}
