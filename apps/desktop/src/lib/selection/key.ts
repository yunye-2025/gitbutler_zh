...
export function readKey(key: SelectedFileKey): SelectedFile {
	...
	if (!isSelectionType(type)) throw new Error('无效的选择键');

	switch (type) {
		case 'commit':
			if (parts.length !== 3) throw new Error('无效的提交键');
			return {
				type,
				path: parts[0]!,
				commitId: parts[1]!,
		...
		case 'branch':
			if (parts.length !== 4) throw new Error('无效的分支键');
			return {
				type,
				path: parts[0]!,
		...
		case 'worktree':
			if (parts.length !== 2) throw new Error('无效的工作区键');
			return {
				type,
				path: parts[0]!,
				stackId: parts[1] === 'undefined' ? undefined : parts[1]
		...
		case 'snapshot':
			if (parts.length !== 2) throw new Error('无效的快照键');
			return {
				type,
				snapshotId: parts[0]!,
				path: parts[1]!
...
export function readStableSelectionKey(key: StableSelectionKey): SelectionId {
	...
	switch (type) {
		case 'commit':
			if (parts.length !== 2) throw new Error('无效的提交键');
			return {
				type,
				commitId: parts[0]!,
				stackId: parts[1] === 'undefined' ? undefined : parts[1]!
		...
		case 'branch':
			if (parts.length !== 3) throw new Error('无效的分支键');
			return {
				type,
		...
		case 'worktree':
			if (parts.length !== 1) throw new Error('无效的工作区键');
			return {
				type,
				stackId: parts[0] === 'undefined' ? undefined : parts[0]
			};
		case 'snapshot':
			if (parts.length !== 1) throw new Error('无效的快照键');
			return {
				type,
				snapshotId: parts[0]!
			};
