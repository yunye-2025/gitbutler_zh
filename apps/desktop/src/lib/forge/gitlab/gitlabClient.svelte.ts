...
export function gitlab(extra: unknown): {
	...
	if (!hasGitLab(extra)) throw new Error('未找到 GitLab 客户端！');
	if (!extra.gitLabClient.api) throw new Error('无法找到 GitLab 客户端');
	if (!extra.gitLabClient.forkProjectId) throw new Error('无法找到分叉项目 ID');
	if (!extra.gitLabClient.upstreamProjectId) throw new Error('无法找到上游项目 ID');
