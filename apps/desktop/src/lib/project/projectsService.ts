...
export class ProjectsService {
	...
	async relocateProject(projectId: string): Promise<void> {
		...
		try {
			...
			chipToasts.success(`项目 ${project.title} 已重新定位`);
			window.location.reload();
		} catch (error: any) {
			showError('重新定位项目失败：', error.message);
		}
	}
	...
	validateProjectPath(path: string) {
		if (/^\\\\wsl.localhost/i.test(path)) {
			const errorMsg =
				'对于 WSL2 项目，请在你的 WSL2 发行版中安装 GitButler 的 Linux 版本';
			console.error(errorMsg);
			showError('请使用 GitButler 的 Linux 版本', errorMsg);
		...
		if (/^\\\\/i.test(path)) {
			const errorMsg =
				'不建议通过网络使用 Git。请在本地克隆仓库，或使用 NET USE 命令映射网络驱动器';
			console.error(errorMsg);
			showError('不直接支持 UNC 路径', errorMsg);
