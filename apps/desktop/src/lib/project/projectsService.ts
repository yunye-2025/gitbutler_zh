...
export class ProjectsService {
	...
	async relocateProject(projectId: string): Promise<void> {
		...
		try {
			...
			chipToasts.success(`项目 ${project.title} 已迁移`);
			window.location.reload();
		} catch (error: any) {
			showError('迁移项目失败：', error.message);
		}
	}
	...
	validateProjectPath(path: string) {
		if (/^\\\\wsl.localhost/i.test(path)) {
			const errorMsg =
				'对于 WSL2 项目，请在 WSL2 发行版内安装 GitButler 的 Linux 版本';
		...
		if (/^\\\\/i.test(path)) {
			const errorMsg =
				'不建议跨网络使用 git。请在本地克隆仓库，或使用 NET USE 命令映射网络驱动器。';
			console.error(errorMsg);
