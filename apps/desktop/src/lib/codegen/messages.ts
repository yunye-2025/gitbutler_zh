...
export function formatMessages(
	...
	for (const [_idx, message] of events.entries()) {
		...
		if (payload.source === 'user') {
			...
			if (payload.data.type === 'assistant') {
				...
				if (Array.isArray(content) && content[0]!.type === 'tool_result') {
					...
					if (!foundToolCall) {
						...
						foundToolCall.result = '工具已完成，但无输出';
			...
			if (payload.type === 'claudeExit' && payload.code !== 0) {
				if (previousEventLoginFailureQuery(events, message)) {
					out.push({
						source: 'claude',
						createdAt: message.createdAt,
						message: `Claude Code 当前未登录。\n\n请在终端运行 \`claude\` 并完成登录流程，以使用 GitButler 的 Claude Code 集成。`,
						toolCalls: [],
						toolCallsPendingApproval: []
					});
				} else {
			...
			if (payload.type === 'unhandledException') {
				out.push({
					source: 'claude',
					message: `执行 Claude 时遇到未处理的异常。\n请确认 Claude Code 的安装位置，并尝试清理上下文。 \n\n\`\`\`\n${payload.message}\n\`\`\``,
					toolCalls: [],
					toolCallsPendingApproval: [],
					createdAt: message.createdAt
				});
			...
			if (payload.type === 'compactFinished') {
				out.push({
					...
					createdAt: message.createdAt,
					subtype: 'compaction',
					message: `上下文压缩完成：${payload.summary}`,
					toolCalls: [],
					toolCallsPendingApproval: []
				});
			}
	...
	function wrapUpAgentSide() {
		...
		for (const toolCall of Object.values(toolCalls)) {
			...
			toolCall.result = '工具调用已中止 - 会话已不再活跃';
		}
		toolCalls = {};
