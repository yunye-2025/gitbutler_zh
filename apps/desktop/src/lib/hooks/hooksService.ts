...
export class HooksService {
	...
	async runPreCommitHooks(projectId: string, changes: DiffSpec[]): Promise<void> {
		const loadingToastId = chipToasts.loading('已开始运行 pre-commit 钩子');
		...
		try {
			...
			chipToasts.removeChipToast(loadingToastId);
			chipToasts.success('pre-commit 钩子执行成功');
		} catch (e: unknown) {
			chipToasts.removeChipToast(loadingToastId);
			throw e;
		}
	...
	async runPostCommitHooks(projectId: string): Promise<void> {
		const loadingToastId = chipToasts.loading('已开始运行 post-commit 钩子');
		...
		try {
			...
			chipToasts.removeChipToast(loadingToastId);
			chipToasts.success('post-commit 钩子执行成功');
		} catch (e: unknown) {
			chipToasts.removeChipToast(loadingToastId);
			throw e;
		}
