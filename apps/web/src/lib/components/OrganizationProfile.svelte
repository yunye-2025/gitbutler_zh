<script lang="ts">
	...
	async function makeUserOwner() {
		...
		try {
			...
			console.error('设置用户为所有者失败：', error);
		} finally {
			isPromoting = false;
		...
		<div class="org-header">
			{#if localOrganization.avatarUrl}
				<img
					...
					alt="{localOrganization.name} 的标识"
					class="avatar"
				/>
			{/if}
			<div class="org-title">
		...
		<div class="side-column">
			{#if localOrganization.inviteCode}
				<div class="section-card invite-code-section">
					<h2 class="section-title">邀请代码</h2>
								...
								<a href="/{member.login}" class="member-link">
									...
									<div class="member-info">
										...
										<span class="member-role">
											{member.name}
											{#if isOwner(member)}
												<span class="badge owner-badge">所有者</span>
											{/if}
										</span>
									</div>
								</a>
...
<Modal bind:this={confirmRemoveUserModal} width="small" onSubmit={removeUser}>
	...
	{#snippet controls(close)}
		...
		<Button style="danger" type="submit" loading={isRemoving}>移除</Button>
	{/snippet}
</Modal>
...
<Modal bind:this={confirmMakeOwnerModal} width="small" onSubmit={makeUserOwner}>
	<p>确定要将该用户设为所有者吗？</p>
	<p class="modal-note">
		所有者拥有完整的组织管理权限，包括成员、项目与设置管理。
	</p>
	{#snippet controls(close)}
		<Button kind="outline" onclick={close}>取消</Button>
		<Button style="pop" type="submit" loading={isPromoting}>设为所有者</Button>
	{/snippet}
</Modal>
