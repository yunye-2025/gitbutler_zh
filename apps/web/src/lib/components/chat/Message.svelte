...
<script lang="ts">
	...
	function getReactionTooltip(users: UserSimple[]) {
		...
		const formatted = users.map((user) => (user.login === thisUsername ? '你' : user.login));
		if (formatted.length < 4) return formatted.map((user) => user).join(', ');
		return (
			formatted
				.slice(0, 3)
				.map((user) => user)
				.join(', ') + `，以及另外 ${formatted.length - 3} 人`
		);
	}
...
>
	...
	<div class="chat-message__data">
		<div class="chat-message__header">
			...
			{#if message.issue}
				{#if message.resolved}
					<Badge style="safe">问题已解决</Badge>
				{:else}
					<Badge style="warning">问题</Badge>
				{/if}
			{/if}
