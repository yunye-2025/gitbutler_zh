<script lang="ts">
	...
	const actionButtonLabel = $derived.by(() => {
		const suffix = messageHandler.message ? ' 并评论' : '';
			...
			<div class="chat-input__actions">
				<div class="chat-input__inner-toolbar">
					...
					<div class="chat-input__inner-toolbar__shrinkable">
						<Button
							...
							width="100%"
						>
							<span style="opacity: 0.4">粘贴或拖放以添加文件</span>
						</Button>
					</div>
				</div>
				...
				<div class="chat-input__action-buttons">
					...
					<Button
						...
						onclick={handleClickSend}>评论</Button
					>
				</div>
			</div>
		</div>
...
{:else}
	<div class="chat-input-notlooged">
		<p class="text-12">🔒 登录后才能参与讨论</p>
		<Button style="pop" onclick={login}>登录后评论</Button>
	</div>
{/if}
