<script lang="ts">
	...
	async function connectToOrganization(organizationSlug: string) {
		...
		try {
			...
			chipToasts.success('项目已连接到组织');
			modal?.close();
		} catch (error) {
			chipToasts.error(
				`连接项目失败：${error instanceof Error ? error.message : '未知错误'}`
			);
		}
	}

	const title = $derived.by(() => {
		if (project?.status !== 'found') return '连接项目';
		return `将 ${project.value.name} 连接到组织`;
	});
...
<Modal bind:this={modal} {title}>
	...
	{:else}
		...
		<div class="selection-header">
			<h4>在 {selectedOrgSlug} 中选择项目</h4>
			<Button style="gray" onclick={() => (selectedOrgSlug = null)}>返回组织列表</Button>
		</div>
			...
			<CardGroup>
						...
						<div class={selectedProjectSlug === orgProject.slug ? 'selected' : ''}>
							<CardGroup.Item onclick={() => selectProject(orgProject.slug)}>
								<div class="project-info">
									...
									{/if}
									<p class="slug">标识：{orgProject.slug}</p>
								</div>
				...
				<div class={isCreatingNew ? 'selected create-new' : 'create-new'}>
					<CardGroup.Item onclick={toggleCreateNew}>
						<div class="project-info">
							<h5 class="text-15 text-bold">创建新项目</h5>
							{#if isCreatingNew}
								<div class="new-project-form">
									<label for="newProjectSlug">项目标识：</label>
									<input
										type="text"
										id="newProjectSlug"
							...
							{:else}
								<p class="description">用此仓库创建新项目</p>
							{/if}
						</div>
						<div class="radio-option">
							<input
