...
<section class="latest-nightly-wrapper">
	...
	{#if latestNightly}
		<div class="nightly-hero">
			<div class="nightly-hero__header">
				...
				<div class="nightly-hero__header-labels">
					...
					<div class="nightly-hero__header-details">
						...
						<span> • </span>
						<span
							>发布于 {new Date(latestNightly.released_at).toLocaleDateString('zh-CN', {
								day: 'numeric',
								month: 'long',
								year: 'numeric'
							})}{' '}
							{new Date(latestNightly.released_at).toLocaleTimeString('zh-CN', {
								hour: '2-digit',
								minute: '2-digit',
								hour12: false
							})}
					...
					<p class="nightly-hero__description">
						抢先体验 GitButler 最新功能。夜间构建会从最新开发代码自动生成，可能包含实验
						特性与缺陷。
					</p>
				</div>
			</div>
			...
			<p class="nightly-warning">
				⚠️ 夜间构建为实验版本，可能不稳定，风险自负。
				<br />
				用于生产请使用<a href="/downloads">稳定版</a>。
			</p>
	...
	{:else}
		<div class="no-nightly">
			<p class="text-16 clr-text-2">当前没有可用的夜间构建。</p>
		</div>
	{/if}
</section>
					...
					<div class="release-row__content">
						...
						<div class="release-row__info">
							<span class="release-row__date">
								{new Date(release.released_at).toLocaleDateString('zh-CN', {
									day: 'numeric',
									month: 'short',
									year: 'numeric'
								})},
								{new Date(release.released_at).toLocaleTimeString('zh-CN', {
									hour: '2-digit',
									minute: '2-digit',
									hour12: false
								})},
