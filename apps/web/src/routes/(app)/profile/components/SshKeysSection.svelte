<script lang="ts">
	...
	async function deleteSshKey(fingerprint: string) {
		...
		const confirmed = confirm(`确定要删除 SSH 密钥“${key.name}”吗？`);
		if (!confirmed) return;
	...
	async function generateSshKeyToken() {
		...
		try {
			...
			if (updatedUser && updatedUser.ssh_key_token) {
				...
				console.error('生成 SSH 密钥令牌失败：未返回令牌');
			}
		} catch (error) {
			console.error('生成 SSH 密钥令牌失败：', error);
		} finally {
...
<CardGroup.Item standalone>
	<div class="ssh-keys">
		{#if loadingSshKeys}
			<div class="loading">正在加载 SSH 密钥...</div>
		{:else if sshKeys.length === 0}
			<div class="no-keys">尚未添加 SSH 密钥</div>
		{:else}
			{#each sshKeys as key}
				<div class="ssh-key">
		...
		>
			<span class="add-key-icon">+</span>
			<span>{generatingSshToken ? '生成中...' : '通过 SSH 添加密钥'}</span>
		</button>
	</div>
</CardGroup.Item>
...
{#if showSshKeyTokenModal}
	<div class="ssh-token-modal-backdrop" onclick={closeSshKeyTokenModal}>
		<div class="ssh-token-modal" onclick={handleModalClick}>
			<h3 class="ssh-token-modal__title">添加你的 SSH 密钥</h3>
			<p class="ssh-token-modal__description">
				在终端运行以下命令，将你的 SSH 密钥添加到 GitButler：
			</p>
			...
			<p class="ssh-token-modal__note">该令牌将在使用后或 5 分钟后过期。</p>
