<script lang="ts">
	...
	async function resendConfirmationEmail() {
		...
		if (response.type === 'error') {
			...
			message = '确认邮件已重新发送，请检查收件箱。';
		}
	}
</script>

<svelte:head>
	<title>GitButler | 重新发送确认邮件</title>
</svelte:head>
...
>
	{#if email}
		<p class="text-13 text-body">
			我们将确认邮件发送至 <i class="clr-text-2">{email}</i>。
		</p>
	{:else}
		<div class="stack-v gap-16">
			<EmailTextbox bind:this={emailTextbox} bind:value={inputEmail} label="邮箱" />
