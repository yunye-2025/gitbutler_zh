...
<div class="invite-container">
	<div class="invite-card">
		...
		{#if !isLoggedIn}
			<p>你已被邀请加入 <strong>{slug}</strong>。</p>
			<p>请登录以继续。</p>
			<Button onclick={goToLogin} style="pop">登录</Button>
		{:else if isJoining}
		...
		{:else if joinError}
			...
			<div class="button-container">
				<Button onclick={handleRetry} style="pop">重试</Button>
			</div>
		{:else if joinSuccess}
			<p>你已成功加入组织。</p>
			<p>正在跳转到组织页面...</p>
		{:else if showConfirmation}
			...
			<p>是否接受此邀请？</p>
			<div class="button-container">
				<Button onclick={handleConfirm} style="pop">加入组织</Button>
			</div>
		{:else}
			<p>正在处理你的邀请...</p>
		{/if}
