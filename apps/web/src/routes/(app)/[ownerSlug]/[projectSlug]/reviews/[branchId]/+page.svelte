...
{#snippet startReview(branch: Branch)}
	{#if (branch.stackSize || 0) > 0 && isBranchAuthor === false}
		<Button style="pop" icon="play" onclick={() => visitFirstCommit(branch)}>开始评审</Button>
	{/if}
{/snippet}
...
<svelte:head>
	{#if isFound(branch?.current)}
		...
		<meta property="og:title" content="GitButler 评审：{branch.current.value?.title}" />
		<meta property="og:description" content="GitButler 代码评审" />
	{:else}
		<title>{data.ownerSlug}/{data.projectSlug}</title>
		<meta property="og:title" content="GitButler 评审：{data.ownerSlug}/{data.projectSlug}" />
		<meta property="og:description" content="GitButler 代码评审" />
	{/if}
</svelte:head>
...
{:else if hasAnyError && combinedLoadable}
	...
	{:else if isError(combinedLoadable)}
		<div class="error-container">
			<h2 class="text-15 text-body text-bold">加载项目数据出错</h2>
			...
			<div class="layout">
				<div class="information">
					<div class="heading">
						...
						{/if}
						<div class="actions">
							<Button icon="copy-small" kind="outline" onclick={copyLocation}>分享链接</Button>
							...
							{:else}
								<AsyncButton
									style="danger"
									kind="outline"
									action={async () => updateStatus(BranchStatus.Closed)}>关闭评审</AsyncButton
								>
							{/if}
						</div>
					</div>
					<InfoFlexRow>
						<Factoid label="状态"><BranchStatusBadge {branch} /></Factoid>
						<Factoid label="提交">
							{#if $user}
								<Minimap
					...
					<div class="summary">
						{#if editingSummary}
							...
							<div class="summary-actions">
								<Button kind="outline" onclick={abortEditingSummary}>取消</Button>
								<AsyncButton style="pop" action={saveSummary}>保存</AsyncButton>
							</div>
						{:else if branch.description}
							...
							{#if branch.permissions.canWrite}
								<div>
									<Button kind="outline" onclick={editSummary}>修改详情</Button>
								</div>
							{/if}
						{:else}
							<div class="summary-placeholder">
								<p class="text-13 clr-text-2">未提供摘要。</p>
								{#if branch.permissions.canWrite}
									<p class="text-12 text-body clr-text-2">
										<em>
											摘要可提供分支目的的背景，帮助团队成员理解其变更。
										</em>
									</p>
									<Button icon="plus-small" kind="outline" onclick={editSummary}>添加摘要</Button
									>
								{/if}
							</div>
						{/if}
