<script lang="ts">
	...
	onMount(async () => {
		try {
			...
			if (!playlistId) {
				throw new Error('无效的播放列表链接');
			}

			playlist = await fetchPlaylistVideos(playlistId);
		} catch (err) {
			error = err instanceof Error ? err.message : '无法加载视频';
			console.error('加载播放列表时出错：', err);
		} finally {
			isLoading = false;
		}
	});
...
<section class="feature-updates">
	<SectionHeader>
		...
		{#snippet buttons()}
			<div class="feature-updates__all-demos">
				<ArrowButton showArrow={false} label="全部演示" onclick={openPlaylist} />
			</div>
	...
	{:else if error}
		<div class="loading-state">
			...
			<p>
				请直接查看我们的
				<a href={PLAYLIST_URL} target="_blank" rel="noopener">YouTube 播放列表</a>
				。
			</p>
		</div>
